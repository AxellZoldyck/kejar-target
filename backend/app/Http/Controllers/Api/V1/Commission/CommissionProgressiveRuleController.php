...
use App\Http\Requests\Commission\ReplaceProgressiveRulesRequest;
use App\Http\Resources\Commission\CommissionSettingResource;
use App\Models\CommissionSetting;
use Illuminate\Http\JsonResponse;
use Illuminate\Http\Request;

class CommissionProgressiveRuleController extends ApiController
{
    public function index(Request $request): JsonResponse
    {
        ...
        return $this->data($setting
            ? (new CommissionSettingResource($setting))->resolve()['progressive_rules']
            : []);
    }

    public function update(ReplaceProgressiveRulesRequest $request, ReviseCommissionSettingAction $action): JsonResponse
    {
        $setting = $action->execute($request->user(), progressives: $request->validated('rules'));
